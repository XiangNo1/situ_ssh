...
$(function(){
	/*展示数据的datagrid表格*/
	$("#datagrid").datagrid({
		url:'${ctx}/customer_findAll.action',
		method:'get',
		fit:true,
		singleSelect:false,
		toolbar:'#toolbar',
		...
		columns:[[    
		     ...
		     {field:'salesperson',title:'所属销售员',width:100,align:'center'},    
...
function openAddDialog() {
	...
	url = "${ctx}/customer_addCustomer.action";
...
function openUpdateDialog() {
	...
	url = "${ctx}/customer_updateCustomer.action";
	$('#form').form("load", row);
}
...
function doSearch(){
	$("#datagrid").datagrid("load",{
		...
		'salesperson' : $("#salespersonSearch").val(),
	})
};
...
function doDelete() {
	...
	$.messager.confirm('确认','您确认想要删除记录吗？',function(r){    
	    if (r){    
	    	$.post(
					"${ctx}/customer_deleteCustomer.action",
					{ids:ids}, 
					function(data) {
						$.messager.progress('close');	// 如果表单是无效的则隐藏进度条
						if(data.status == 0) {
		      ...
		      客户名称：<input style="width: 100px;" type="text" id="nameSearch"></input>
		       客户等级：<input type="text" id="levelSearch" class="easyui-combobox"
					 data-options="
					 	...
					 	panelHeight:'auto'
					 	 "/>
			所属销售员：<input style="width: 100px;" type="text" id="salespersonSearch"></input>
	...
	<div id="dialog" style="width:650;height:280,padding: 10px 20px"  modal="true" closed="true">
		<form action="" id="form" method="post">
			...
			<table cellspacing="8px">
		   		...
		   		<tr>
		   			<td>所属销售员：</td>
		   			<td><input type="text" id="salesperson" name="salesperson" /></td>
		   			<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
		   			<td>电话：</td>
